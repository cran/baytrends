...
flwAveragePred <- function(pdat, pgam, normPct) {
  ...
  pdflw   <- expand.grid(pdat$date,normQ) # create a vector of flow quantiles of
                                             # length mn
  names(pdflw) <- c('date','normQ')
  
  f.dat <- merge(pdat,pdflw,by=c('date'),all=TRUE) 
                     # create a prediction data set with m flow records per date
  f.dat <- f.dat[order(f.dat$date,f.dat$normQ),]
  f.dat$flw_sal <- f.dat$normQ*f.dat$flw_sal.sd    
          # get flow quantiles as product of normal quantiles as flow SD
  f.dat$pred    <- predict(pgam,newdata=f.dat)     # compute flow predictions
  fav.lp  <- predict(pgam,newdata=f.dat,type="lpmatrix")  
         # get prediction matrix for flow predictions 
  VCmat   <- pgam$Vp                  
          # variance-covariance matrix of coefficents
  ...
  fa.mat  <- matrix(favec,nrow=n,byrow=TRUE)  
                   # put the long vector into a matrix
  ...
  fa.lp         <- fa.mat%*%fav.lp  
          #multiply flow average matrix times linear predictors
  pdat$fav.se  <- sqrt(diag(fa.lp%*%VCmat%*%t(fa.lp)))   
        # compute Std. Err. by usual rules
