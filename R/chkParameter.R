...
.chkParameter <- function(df,parameterFilt=parameterFilt, parameterList) {
  ...
  # NULL trap ... 
  ## user parameter list and derive a list
  if (length(parameterFilt)==0) {
  msg <- "No parameter filter specified by user--master parameter list applied."
    warning(msg)
  ...
  # error trap ... stop if
  ## no values from user selected parameterFilt are in df
  ...
  # error trap ... stop if 
  ## no values from user selected parameterFilt are in parameter list
  ...
  # identify variables in df that are 
  ## not in  station, date, layer and valid parameter list
  tmp <- names(df[!(names(df) %in% c(c("station", "date", "layer")
                                     , parameterList$parm))])
  if (!length(tmp)==0) {
  ...
  # identify parameters in user parameter filter list that are 
  ## not in valid parameter list
  tmp <- parameterFilt[!(parameterFilt %in% parameterList$parm)]
  if (!length(tmp)==0) {
    warning(paste("User selected parameter filter value not used: "
                  ,paste(tmp, collapse = " ")))
  }
...
#   if (!length(tmp)==0) {
#     warning(paste("Following variable found in user selected parameter filter 
  #is not",
...
#   }
  ...
  df <- df[, names(df) %in% c(c("station", "date", "layer")
                              , parameterList$parm)]
  ...
  # attribute list of id variables
  attr(df, "othVar") <-  names(df[!(names(df) %in% c(attr(df,"idVar")
                                                     , attr(df,"depVar")))])
